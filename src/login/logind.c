...
static int manager_connect_console(Manager *m) {
        ...
        if (m->console_active_fd < 0) {
                ...
                if (errno == ENOENT) {
                        log_warning_errno(errno, "System has /dev/tty0 but /sys/class/tty/tty0/active which is broken, ignoring: %m");
                        return 0;
                }
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to subsribe to signal: %m");
